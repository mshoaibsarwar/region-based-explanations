...
│   ├── rshap.py            # RegionSegmentation, GIR, FixedRegionSHAP
...
├── notebooks/
│   └── rshap_modelnet10_pointnet_final.ipynb   # Interactive notebook version
├── figures/
│   ├── pipeline.png
...
| `src/rshap.py` | Core R-SHAP framework: `RegionSegmentation` (FPS+Voronoi, K-Means, Spectral), `GeometricInterpolationReference` (GIR), and `FixedRegionSHAP` (coalition sampling, OLS regression, efficiency constraint) |
...
Open `notebooks/rshap_modelnet10_pointnet_final.ipynb` in Google Colab and run all cells. The notebook is self-contained and handles data download, model training, and the complete R-SHAP analysis.
...
@article{sarwar2026rshap,
  title={Region-Based SHAP for 3D Point Cloud Explainability},
  author={Sarwar, Muhammad Shoaib and Pham, Le Hoang and Ahmed, Faizan},
  year={2026}
}
```
